...
class mursH5FileReader:
    ...
    def getDataRecord(self, elemNumber):
        ...
        for key in self.sn:
            ...
            sensorRec['fine_gain_act'] = long(ds['fine_gain'][elemNumber])
            sensorRec['fine_gain_set'] = long(ds['fine_gain_set'][elemNumber])
            sensorRec['gain_stab_enable'] = long(ds['gain_stab_enable'][elemNumber])
            sensorRec['gain_stab_max'] = long(ds['gain_stab_max'][elemNumber])
            sensorRec['gain_stab_mid'] = long(ds['gain_stab_mid'][elemNumber])
            sensorRec['gain_stab_min'] = long(ds['gain_stab_min'][elemNumber])
            sensorRec['hv_act'] = long(ds['hv_act'][elemNumber])
            sensorRec['hv_actual'] = ds['hv_actual'][elemNumber]
            sensorRec['hv_set'] = long(ds['hv_set'][elemNumber])
            sensorRec['livetime'] =long( ds['livetime'][elemNumber])
            sensorRec['lld_set'] = long(ds['lld_set'][elemNumber])
            sensorRec['lt_preset'] = long(ds['lt_preset'][elemNumber])
